    ...
    public static class DatabaseSocket
    {
        ...
        public static DataTable GetHistoryOfLastDay()
        {
            ...
            try
            {
                ...
                _myConnection.Open();
                SqlCommand command = new SqlCommand(@"SELECT * FROM [dbo].[items] WHERE [timestamp]>=DATEADD(day,-1,GETDATE()) ORDER BY timestamp DESC", _myConnection);
        ...
        public static DataTable GetHistoryOfLastHour()
        {
            ...
            {
                _myConnection.Open();
                SqlCommand command = new SqlCommand(@"SELECT * FROM [dbo].[items] WHERE [timestamp]>=DATEADD(hour,-1,GETDATE()) ORDER BY timestamp DESC", _myConnection);
                history.Load(command.ExecuteReader());
                _myConnection.Close();
